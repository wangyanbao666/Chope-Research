...
def main():
    ...
    g_counter=0
    repeat=6
    chop_probs=[np.round(x,5) for x in np.arange(40,100.1,0.1)]
    chop_probs=np.repeat(chop_probs,repeat)    
    # hyperparameters = [[1500,50,3,25,2,1],[1500,50,4,25,2,1],[1500,50,5,25,2,1],[1500,50,6,25,2,1],[1500,50,7,25,2,1],[1500,50,8,25,2,1]]
    hyperparameters=[[1500, 55, 4, 25, 2, 1]]
    ...
    for hyperparam in hyperparameters:
        ...
        df['chop_prob_round1'] = x[::3]
        df['avg_time_round1'] = y[::3]
        df['chop_prob_round2'] = x[1::3]
        df['avg_time_round2'] = y[1::3]
        df['chop_prob_round3'] = x[2::3]
        df['avg_time_round3'] = y[2::3]
        ...
        plt.savefig('outflow/Run number {} outflow rate against probability(40-100)'.format(g_counter),  bbox_inches='tight', dpi=300)
        plt.clf()
        ...
        # plt.figtext(0.93,0.6,"Seats:{}\nMax_inflow:{}\nEat_time:{}\nStalls:{}\nFood_prep:{}".format(hyperparam[1],hyperparam[2],hyperparam[3],hyperparam[4],hyperparam[5]),fontsize=6,bbox=dict(facecolor='none', edgecolor='black'))
        # plt.savefig('inflow/Run number {} Satisfaction against time'.format(g_counter), bbox_inches='tight',dpi=300)
        # plt.clf()
