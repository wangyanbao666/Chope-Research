...
def seat_taken(seats):
    num_seat_taken=np.sum(seats[:,0])
    crowdedness= num_seat_taken/len(seats)
    return num_seat_taken, crowdedness

# @numba.jit
def get_status(customers):
    num_queue = np.sum(customers[(customers[:,0]==1) & (customers[:,2]==1),2])
    num_look_seat = np.sum(customers[(customers[:,0]==1) & (customers[:,2]==2),2])/2
    num_eating = np.sum(customers[(customers[:,0]==1) & (customers[:,2]==3),2])/3
    num_in_hawker=np.sum(customers[customers[:,0]==1,0])
    print('nun_queue:',num_queue)
...
def update_hawker_status(customers,seats,feed_time,t):
    # leaving_customers=np.zeros()
    customers[(customers[:,0]==1) & (customers[:,2]==2),3]+=1
    customers[(customers[:,0]==1) & (customers[:,2]==3),5]-=1
    customers[(customers[:,0]==1) & (customers[:,2]==1),4]-=1

    # customers[(customers[:,0]==1) & (customers[:,4]==0),2]=0
    customers[(customers[:,0]==1) & (customers[:,2]==1) & (customers[:,4]==0),2]=customers[(customers[:,0]==1) & (customers[:,2]==1) & (customers[:,4]==0),1]+2
    customers[(customers[:,0]==1) & (customers[:,2]==1) & (customers[:,4]==0) & (customers[:,1]==1),5]=feed_time

    print(customers)
    # try:
    leaving_customers=[]
    leaving_customers_1=customers[(customers[:,0]==1) & (customers[:,2]==3) & (customers[:,5]==0),3]
    leaving_customers_2=customers[(customers[:,0]==1) & (customers[:,2]==3) & (customers[:,5]==0),6]
    appe=np.zeros((leaving_customers_1.shape[0],1))
    appe[:]=t
    for i in range(len(leaving_customers_1)):
        leaving_customers+=[[leaving_customers_1[i],leaving_customers_2[i],t]]
    # except:
    #     leaving_customers=[]
    customers[(customers[:, 0] == 1) & (customers[:, 2] == 3) & (customers[:, 5] == 0), 0] = 2
...
def main():
    ...
    for hyperparam in hyperparameters:
        ...
        for chop_prob in chop_probs:
            ...
            avg_ouflow_rate=len(customer_data)/simulation_length
        ...
        plt.scatter(x,y,s=3)
        plt.title("Average outflow rate of customers against choping probability")
        ...
        plt.ylabel('Average outflow rate')
        plt.savefig('test/Run number {} Waiting time against probability'.format(g_counter),  bbox_inches='tight', dpi=300)
        plt.clf()
        ...
        plt.figtext(0.93,0.6,"Seats:{}\nMax_inflow:{}\nEat_time:{}\nStalls:{}\nFood_prep:{}".format(hyperparam[1],hyperparam[2],hyperparam[3],hyperparam[4],hyperparam[5]),fontsize=6,bbox=dict(facecolor='none', edgecolor='black'))
        plt.savefig('test/Run number {} Satisfaction against time'.format(g_counter), bbox_inches='tight',dpi=300)
        plt.clf()
