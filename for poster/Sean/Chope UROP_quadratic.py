...
def variable_prob():
    ...
    # hyperparameters = [[max_custs,70,5,25,2,1]]
    ...
    for hyperparam in hyperparameters:
        ...
        for t in range(simulation_length):
            ...
            waiting_time = sum([cust_time[0] + cust_time[1] for cust_time in leaving_customers])
            waiting_time_data.append(waiting_time)


        avg_waiting_time= [np.mean(waiting_time_data[i:i+running_average]) for i in range(len(waiting_time_data)-running_average)]
        trnc_probs = prob_data[:-running_average]

        print(len(avg_waiting_time),len(trnc_probs))
        ...
        plt.figtext(0.93,0.6,"Seats:{}\nMax_inflow:{}\nEat_time:{}\nStalls:{}\nFood_prep:{}\nSimulation Length:{}\nRunning Average:{}".format(hyperparam[1],hyperparam[2],hyperparam[3],hyperparam[4],hyperparam[5],simulation_length,running_average),fontsize=6,bbox=dict(facecolor='none', edgecolor='black'))
        plt.xlabel('Choping probability')
        plt.ylabel('Outflow Rate')
        plt.savefig('time\quadratic\Run number2 {} Choping Probability against waiting time'.format(g_counter),  bbox_inches='tight', dpi=300)
        plt.clf()
        print(1)
        g_counter+=1
